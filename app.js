...
const els = {
  ...
  loadoutDialog: document.getElementById("loadoutDialog"),
  loadoutDialogTitle: document.getElementById("loadoutDialogTitle"),
  loadoutText: document.getElementById("loadoutText"),
  loadoutMessage: document.getElementById("loadoutMessage"),
  copyLoadoutButton: document.getElementById("copyLoadoutButton"),
  downloadLoadoutButton: document.getElementById("downloadLoadoutButton"),
  applyLoadoutButton: document.getElementById("applyLoadoutButton"),
  closeLoadoutDialog: document.getElementById("closeLoadoutDialog"),
};
...
function currentLoadoutPayload() {
  const levels = {};
  PERKS
    .filter((perk) => (state.levels[perk.id] || 0) > 0)
    .sort(perkBoardSort)
    .forEach((perk) => {
      levels[perk.id] = normalizedLevel(state.levels[perk.id]);
    });
  ...
  return {
    ...
    pointLimit: hasPointLimit() ? state.pointLimit : null,
    pointsRequired: totalSpent(),
    levels,
  };
}

function encodeLoadout(payload) {
...
function readLoadout(text) {
  const trimmed = String(text || "").trim();
  if (!trimmed) throw new Error("Paste a loadout first.");
  const payload = trimmed.startsWith("{") ? JSON.parse(trimmed) : decodeLoadoutCode(trimmed);
  const rawLevels = payload?.levels;
  if (!rawLevels || typeof rawLevels !== "object" || Array.isArray(rawLevels)) {
    throw new Error("That loadout does not contain perk levels.");
  }

  const levels = {};
  Object.entries(rawLevels).forEach(([id, rawLevel]) => {
    if (!perkById.has(id)) return;
    const level = normalizedLevel(rawLevel);
    if (level > 0) levels[id] = level;
  });

  return {
    levels,
    pointLimit: payload.pointLimit === null || payload.pointLimit === undefined ? null : parsePointLimit(payload.pointLimit),
  };
}

function setLoadoutMessage(text, isError = false) {
  els.loadoutMessage.textContent = text;
  els.loadoutMessage.classList.toggle("is-error", isError);
}

function setLoadoutMode(mode) {
  const isExport = mode === "export";
  els.loadoutDialog.dataset.mode = mode;
  els.loadoutDialogTitle.textContent = isExport ? "Export Loadout" : "Import Loadout";
  els.copyLoadoutButton.hidden = !isExport;
  els.downloadLoadoutButton.hidden = !isExport;
  els.applyLoadoutButton.hidden = isExport;
  setLoadoutMessage("");
}
...
async function copyLoadout() {
  const text = els.loadoutText.value.trim();
  if (!text) {
    setLoadoutMessage("Nothing to copy.", true);
    return;
  }
  try {
    ...
    setLoadoutMessage(copied ? "Copied." : "Copy failed.", !copied);
  }
}
...
function applyLoadout() {
  try {
    const loadout = readLoadout(els.loadoutText.value);
    state.levels = loadout.levels;
    state.pointLimit = loadout.pointLimit;
...
els.exportLoadoutButton.addEventListener("click", () => openLoadoutDialog("export"));
els.importLoadoutButton.addEventListener("click", () => openLoadoutDialog("import"));
